...
int main(){
    ...
    // //         memcpy(end, &arr[i][strlen(arr[i]) - 1], sizeof(char));
    // //         memcpy(begin, &arr[i][0], 1);
    ...
    for(int i = 0; i < n; ++i){
        cin >> strs[i];
        if(strs[i].length() > 10){
            cout << strs[i].substr(0, 1) + to_string(strs[i].length() - 2) + strs[i].substr(strs[i].length() - 1, 1) << endl;
        }else{
            cout << strs[i] << endl;
        }
    }
